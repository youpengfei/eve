...
def get_branch_list(project):
    ...
    command = ' && '.join(cmd)
    readlines = subprocess.Popen(
        command, shell=True, stdout=subprocess.PIPE).stdout.readlines()
    remote_prefix = 'origin/'
    remote_head_prefix = 'origin/HEAD'

    brand_list = []
    for readline in readlines:
        line = str(readline, encoding='utf-8')
        if line.__contains__(remote_prefix) and not line.__contains__(remote_head_prefix):
...
def update_repo(project, branch='master'):
    ...
    if os.path.exists(do_git):
        ...
        cmd = ['mkdir -p %s ' % git_dir,
               'cd %s ' % git_dir,
               '/usr/bin/env git clone -q %s .' % project.repo_url,
               '/usr/bin/env git checkout -q %s' % branch]
        command = ' && '.join(cmd)
        app.logger.info(command)
        popen = subprocess.Popen(command, shell=True)
...
def update_repo_by_path(source_path, repo_url, branch='master'):
    ...
    if os.path.exists(do_git):
        ...
        cmd = ['mkdir -p %s ' % source_path,
               'cd %s ' % source_path,
               '/usr/bin/env git clone -q %s .' % repo_url,
               '/usr/bin/env git checkout -q %s' % branch]
        command = ' && '.join(cmd)
        app.logger.info(command)
        popen = subprocess.Popen(command, shell=True)
...
def get_project_full_name(repo_url):
    if not repo_url:
        return None
    else:
        return repo_url[repo_url.rfind(':') + 1:-4]
...
def update_to_version(task):
    ...
    update_repo_by_path(source_path=workspace,
                        ...
                        branch=task.branch)
    cmd = ['cd %s ' % workspace, '/usr/bin/env git reset -q --hard %s' %
           task.commit_id]
    command = ' && '.join(cmd)
